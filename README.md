...
自动监控你关注的**微博**大V的**全部动态（发帖 / 转发）**，新动态在 **5~15 分钟内**推送到你的**微信**（PushPlus 公众号，免费每天 200 条），并提供简洁的本地网页用于查看动态、增删大V。

> ⚠️ **雪球暂不支持**：雪球主站接口有阿里云 WAF 反爬（JS 验证），本机和 GitHub 服务器实测均被拦截，纯程序/自动化浏览器暂时抓不到雪球数据。待后续用更深入的方案（如云服务器 + 专门反爬对抗）再接入，相关代码与配置已预留。
...
├── config.json                 # 配置：轮询间隔、推送渠道、大V列表
├── .env.local                  # 密钥（Cookie、PushPlus Token），已被 git 忽略
├── core/                       # 核心：抓取、去重、推送、存储、Git
│   ├── collectors/             #   雪球 / 微博 采集器
│   ├── pipeline.py             #   主流程：抓取→推送→存储
│   ├── notifier.py             #   PushPlus（默认）/ 企业微信（可选）
│   └── ...
...
### 第 2 步：GitHub 公开仓库（24 小时兜底用）
在 [github.com](https://github.com) 新建一个 **Public** 公开仓库（公开仓库的 Actions 免费不限时长），把本目录代码推送上去：
\`\`\`
git remote add origin https://github.com/<你的用户名>/v-monitor.git
git branch -M main
git push -u origin main
\`\`\`
> 若不想公开，可建私有仓库，但免费分钟数只够约 60 分钟轮询一次（实时性差）。

### 第 3 步：开通微信推送（PushPlus，约 2 分钟）
1. 打开 [PushPlus 官网](https://www.pushplus.plus)，**微信扫码登录**，并**关注「PushPlus」公众号**（消息通过它发到你的微信）
2. 登录后首页会显示你的 **token**（一长串字符），复制它
3. 把 token 填到本目录的 \`.env.local\`（复制 \`.env.local.example\` 改名为 \`.env.local\`）：
   \`\`\`
   PUSHPLUS_TOKEN=你的token
   \`\`\`
   并在 GitHub 仓库 Settings → Secrets → Actions 里新建同名 Secret \`PUSHPLUS_TOKEN\`。
> 免费每天 200 条，日常足够；如某天大V特别活跃超量，当天剩余推送会暂停、次日恢复。
> 可选：也可用**企业微信应用消息**（免费无条数限制，需注册企业微信；见 \`core/notifier.py\` 与 \`.env.local.example\` 注释）。
...
3. 粘贴到终端，脚本会自动提取 Cookie 并保存到 \`.env.local\`（不会被提交），并提示上传到 GitHub Secrets
...
- 微博：自动搜索匹配，列出同名账号（同名很多，**请人工确认**，选错会监控错人）
- 雪球：当前受 WAF 反爬限制暂不可用（见上方说明），无需解析
...
| 设置 | 推送渠道/密钥状态、轮询间隔、最近运行结果、日志、允许局域网访问、发送测试消息 |
...
**收不到微信推送？**
依次检查：设置页各密钥是否「已配置」（重点 PushPlus Token）→ 确认已关注「PushPlus」公众号 → 点击「发送测试消息」→ 查看日志页的错误信息。

**PushPlus 今天不推送了？**
免费额度每天 200 条，超出后当天暂停、次日恢复。若大V特别活跃可考虑升级 PushPlus 会员，或切换企业微信渠道（无条数限制）。
...
**数据会公开吗？**
公开仓库里会包含抓取到的大V动态（大V发布的内容本身是公开的）和监控状态。密钥（Cookie、PushPlus Token）全部放在 GitHub Secrets，不会进仓库。若介意公开，可改用私有仓库（推送间隔会变长）或升级云服务器。
...
- 推送：默认 PushPlus（\`config.json\` 的 \`push.channel\`），可在 \`core/notifier.py\` 自由扩展。
